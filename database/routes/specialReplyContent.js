...
const getGitHubUrl = (filename) => {
  return `https://github.com/${GITHUB_CONFIG.owner}/${GITHUB_CONFIG.repo}/raw/master/public/media_special_reply/${filename}`;
};
...
const uploadToGitHub = async (filename, fileBuffer) => {
  try {
    ...
    const apiPath = `public/media_special_reply/${filename}`;
    ...
    const githubUrl = `https://github.com/${GITHUB_CONFIG.owner}/${GITHUB_CONFIG.repo}/raw/${defaultBranch}/public/media_special_reply/${filename}`;
...
const upload = multer({ 
  ...
  fileFilter: function (req, file, cb) {
	...
	// Restrict image types to jpg, jpeg, png, gif only
	const allowedImageTypes = ['image/jpeg', 'image/jpg', 'image/png', 'image/gif'];
  
	// Restrict video types to mp4 only
...
const getNextCategorySequence = async () => {
  const query = `SELECT COALESCE(MAX(SEQUENCE), 0) + 1 as nextSequence FROM special_reply`;
  const [results] = await pool.execute(query);
  return results[0].nextSequence;
};

const getNextItemSequence = async (categoryId) => {
  const query = `SELECT COALESCE(MAX(SEQUENCE), 0) + 1 as nextSequence FROM special_reply_sub WHERE CATEGORY_ID = ?`;
  const [results] = await pool.execute(query, [categoryId]);
  return results[0].nextSequence;
};

const validateCategoryExists = async (categoryId) => {
  const query = `SELECT PKKEY FROM special_reply WHERE PKKEY = ?`;
  const [results] = await pool.execute(query, [categoryId]);
...
// ===== CATEGORY ROUTES (special_reply - MAIN TABLE) =====

// Get all categories
router.get('/categories', async (req, res) => {
  const query = `
    SELECT PKKEY, KEYWORDS, SEQUENCE, CREATED_AT, UPDATED_AT
    FROM special_reply 
...
router.post('/categories', async (req, res) => {
  ...
  try {
    ...
    const insertQuery = `
      INSERT INTO special_reply (KEYWORDS, SEQUENCE, CREATED_AT, UPDATED_AT) 
      VALUES (?, ?, ?, ?)
    `;
...
router.delete('/categories/:id', async (req, res) => {
  ...
  try {
    ...
    const checkQuery = `SELECT PKKEY, KEYWORDS FROM special_reply WHERE PKKEY = ?`;
    ...
    const countQuery = `SELECT COUNT(*) as itemCount FROM special_reply_sub WHERE CATEGORY_ID = ?`;
    ...
    if (itemCount > 0) {
      const deleteItemsQuery = `DELETE FROM special_reply_sub WHERE CATEGORY_ID = ?`;
      await connection.execute(deleteItemsQuery, [id]);
    }

    // Delete the category
    const deleteCategoryQuery = `DELETE FROM special_reply WHERE PKKEY = ?`;
...
router.put('/categories/update-sequence', async (req, res) => {
  ...
  try {
    ...
    for (const update of updates) {
      ...
      const sql = 'UPDATE special_reply SET SEQUENCE = ? WHERE PKKEY = ?';
...
router.get('/categories/with-counts', async (req, res) => {
  const query = `
    SELECT 
      ...
      c.CREATED_AT,
      c.UPDATED_AT,
      COUNT(srs.PKKEY) as CONTENT_COUNT
    FROM special_reply c
    LEFT JOIN special_reply_sub srs ON c.PKKEY = srs.CATEGORY_ID
    GROUP BY c.PKKEY, c.KEYWORDS, c.SEQUENCE, c.CREATED_AT, c.UPDATED_AT
...
// ===== CONTENT ROUTES (special_reply_sub - SUB TABLE) =====
...
router.get('/', async (req, res) => {
  const query = `
    SELECT 
      ...
      srs.SEQUENCE, 
      srs.CATEGORY_ID,
      sr.KEYWORDS as CATEGORY_KEYWORDS
    FROM special_reply_sub srs
    LEFT JOIN special_reply sr ON srs.CATEGORY_ID = sr.PKKEY
    ORDER BY sr.SEQUENCE ASC, srs.SEQUENCE ASC
  `;
...
router.put('/update-sequence', async (req, res) => {
  ...
  try {
    ...
    for (let i = 0; i < updates.length; i++) {
      ...
      const sql = 'UPDATE special_reply_sub SET SEQUENCE = ? WHERE PKKEY = ?';
...
router.post('/text', async (req, res) => {
  ...
  try {
    ...
    const insertQuery = `
      INSERT INTO special_reply_sub (TYPE, TEXT_CONTENT, CATEGORY_ID, SEQUENCE, CREATED_AT, UPDATED_AT) 
      VALUES (1, ?, ?, ?, ?, ?)
    `;
...
router.post('/image', (req, res) => {
  ...
  uploadSingle(req, res, async function (err) {
    ...
    try {
      ...
      const insertQuery = `
        INSERT INTO special_reply_sub (TYPE, MEDIANAME, CATEGORY_ID, SEQUENCE, CREATED_AT, UPDATED_AT) 
        VALUES (2, ?, ?, ?, ?, ?)
      `;
...
router.post('/video', (req, res) => {
  ...
  uploadSingle(req, res, async function (err) {
    ...
    try {
      ...
      // FIX: Added all required parameters including CREATED_AT and UPDATED_AT
      const [results] = await pool.execute(insertQuery, [
        file.filename, 
        categoryId,
        nextSequence,
...
router.put('/:id', upload.fields([{ name: 'image', maxCount: 1 }, { name: 'video', maxCount: 1 }]), async (req, res) => {
  ...
  try {
    ...
    const selectQuery = `SELECT TYPE, MEDIANAME, CATEGORY_ID FROM special_reply_sub WHERE PKKEY = ?`;
...
router.delete('/:id', async (req, res) => {
  ...
  try {
    const selectQuery = `SELECT MEDIANAME FROM special_reply_sub WHERE PKKEY = ?`;
    const [results] = await pool.execute(selectQuery, [id]);
    ...
    const deleteQuery = `DELETE FROM special_reply_sub WHERE PKKEY = ?`;
...
router.get('/media/:filename', async (req, res) => {
  ...
  try {
    ...
    if (isVideo) {
      const rawUrl = `https://raw.githubusercontent.com/${GITHUB_CONFIG.owner}/${GITHUB_CONFIG.repo}/master/public/media_special_reply/${filename}`;
